...
export class InstructorsService {
    ...
    const result = await this.instructorModel.aggregate([
      ...
      // ✅ Shape response
      {
        $project: {
          ...
          email: 1,
          mobile: 1,
          mobileNumber: { $ifNull: ['$mobileNumber', '$mobile'] },
          dob: 1,
          postCode: 1,
          transmissionType: 1,
          description: 1,
          ...
          availabilityWeeks: '$profile.availabilityWeeks',
  ...
  async handleLearnerRefund(order, slot, hours) {
    ...
    const { totalAmount } = await this.handleCreditConsumption.call(
      this,
      order.learnerId,
      hours,
      pricePerHour,
    ...
    // await wallet.save();
    if (wallet) {
      wallet.walletBalance = (wallet.walletBalance ?? 0) + totalAmount;
      await wallet.save();
    }
